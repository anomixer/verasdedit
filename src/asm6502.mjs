...
export const assemble6502 = (lines, startAddress = 0x2000, extraLabels = {}) => {
  ...
  const getEncodedBytes = (instr, operand, currentPc, currentLabels) => {
    ...
    if (instr === "CPX") {
      if (operand.startsWith("#")) return [0xE0, resolveVal(operand)]
      const val = resolveVal(operand)
      if (val <= 0xFF) return [0xE4, val & 0xFF]          // zero-page
      return [0xEC, val & 0xFF, (val >> 8) & 0xFF]         // absolute
    }
    if (instr === "CPY") {
      if (operand.startsWith("#")) return [0xC0, resolveVal(operand)]
      const val = resolveVal(operand)
      if (val <= 0xFF) return [0xC4, val & 0xFF]          // zero-page
      return [0xCC, val & 0xFF, (val >> 8) & 0xFF]         // absolute
    }
